    ...
    login(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!user)
                    return res.status(401).json({ status: 404, message: 'Server internal error' });
                // if exists validate hash password and create token
                if (yield bcrypt.compareSync(req.body.password, user.password)) {
                    const bearer = yield (0, getToken_1.createToken)(user, res);
                    const token = yield token_model_1.Token.create({
                        token: bearer,
                        ip: req.ip,
                    ...
                    let xser = yield user_model_1.User.findOne({
                        where: {
                            id: user.id
                        },
                        attributes: [
                ...
                return res.status(200).json({ status: 200, data: user, token: token_model_1.Token, message: 'Login Successfully' });
            }
            catch (e) {
                console.log(e);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (user && profile) {
                    ...
                    const secure_code = yield verification_code_model_1.VerificationCode.create({
                        UserId: user === null || user === void 0 ? void 0 : user.id,
                        is_revoked: 0,
                        code: code,
                        is_used: 0
                    ...
                    const mailOptions = {
                        from: '"Mims team" <no-reply@mims.com>',
                        to: user.email,
                        subject: 'Verification code for Mims',
                        text: 'Verification code for Mims',
                    ...
                    smtp_1.transporter.sendMail(mailOptions, (err, info) => {
                        if (err) {
                            console.log(err);
                            return;
                        }
    ...
    verify(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const v_code = yield verification_code_model_1.VerificationCode.findOne({
                    where: {
                        [sequelize_1.Op.and]: [{ is_used: 0 }, { is_revoked: 0 }, { code: code }, { UserId: user === null || user === void 0 ? void 0 : user.id }]
                    }
                });
                if (v_code) {
                    user.verified = 1;
